...
@section('content')
    ...
    <div>
        <div class="row col-md-12" style="display: inline-block;padding-left:12px;">
            <h4>
                <b> U-PREPARE | Project Implementation Report :- Procurement</b>
            ...
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active"><a href="#">Project Implementation Report :- Procurement </a></li>
                </ol>
    ...
    <div class="x_panel">
        ...
        <div id="tableExportData" class="x_content">
                <h3 class="pdf-heading text-center" style="text-decoration:underline;"> U-PREPARE | Project Implementation Report - Procurement</h3>
            ...
            <table id="datatable" class="table table-striped table-bordered" style="width:100%" >
                <thead>
                    <tr>
                        ...
                        <th class="text-center" colspan="2">Procurement</th>
                    </tr>
                    ...
                    @if(count($data) > 0)
                        @foreach($data as $key => $d)
                            ...
                            <tr>
                                ...
                                <td style="width:17%;" >
                                    <a data-toggle="tooltip" data-placement="top" title="{{ $d->name }} / {{$d->category->name}}" style="color:blue;" href="{{ route('project.view', $d->id) }}">
                                        {{ $truncated_text }}/{{ $d->category->name }}
                                    </a>
                                    <br>
                                ...
                                <td style="font-size:17px !important;">
                                    <b>Method </b> {{ $d->defineProject->method_of_procurement ?? 'N/A'  }}
                                    <br>
                                    <b>Est. Budget:</b> @if(isset($d->estimate_budget) && $d->estimate_budget)<span class="curr">{{ $d->estimate_budget }}</span>@else N/A @endif
                                    <br>
                                    <b>Tender Fee:</b> @if(isset($d->defineProject->bid_fee) && $d->defineProject->bid_fee)<span class="curr">{{ $d->defineProject->bid_fee }}</span>@else N/A @endif
                                    <br>
                                    <b>Earnest Money Deposit:</b> @if(isset($d->defineProject->earnest_money_deposit) && $d->defineProject->earnest_money_deposit)<span class="curr">{{ $d->defineProject->earnest_money_deposit }}</span>@else N/A @endif
                                    <br>
                                    <b>Bid Validty:</b> {{ $d->defineProject->bid_validity ?? 'N/A' }}
                                    <br>
                                    <b>Bid Completion Period:</b> {{ $d->defineProject->bid_completion_days ?? 'N/A' }}
                                    <br>
                                </td>
                                <td style="width:200px;" class="project_progress">
                                    <div class="progress progress_sm">
                                        <div class="progress-bar bg-green ui-progressbar ui-corner-all ui-widget ui-widget-content" role="progressbar" style="width:{{ $d->weightCompleted }}%;" data-transitiongoal="100" aria-valuemin="0" aria-valuemax="100" aria-valuenow="0">
                                            <div class="ui-progressbar-value ui-corner-left ui-widget-header" style="display: none; width: 5.96296px;"></div>
                                        </div>
                                    </div>
                                    <small>{{ $d->weightCompleted }}% Complete</small>
                                </td>
                            <!--<td style="text-align:center;">-->
                            <!--   @if(($d->contract->status ?? '') == "1")-->
                            <!--            Ongoing-->
                            <!--    @elseif(($d->contract->status ?? '') == "0")-->
                            <!--            Cancalled-->
                            <!--    @else-->
                            <!--            Pending-->
                            <!--    @endif-->

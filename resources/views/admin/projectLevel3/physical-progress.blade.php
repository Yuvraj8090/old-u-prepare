...
@section('content')
            ...
            <div class="row align-items-center">
                ...
                <div class="col-2">
                    @if(auth()->user()->role->level == "THREE")
                        <button type="button" class="btn btn-primary pull-right" data-toggle="modal" data-target="#exampleModal">Update progress</button>
                    @endif
                </div>
            </div>
                    ...
                    @if($data->contract->physical_progress->count())
                        @foreach($data->contract->physical_progress as $k => $prog)
                            <tr>
                                ...
                                <td>
                                    @if(auth()->user()->role->level == "THREE")
                                        <a href="" class="btn btn-md btn-info btn-edit" data-toggle="modal" data-target="#editModal" data-info='@json($prog)' data-activity="{{ $prog->activity->slug }}" data-stage="{{ $prog->stage->slug }}">
                                            <i class="fa fa-edit mr-1"></i>
                                            Edit
                                        </a>
                                        <a href="" class="btn btn-delete btn-danger mr-0" data-id="{{ $prog->id }}">
    ...
    {{-- Edit Progres Modal --}}
    <form autocomplete="off" id="editform" data-method="POST" data-action="{{ route('contract.physical.save') }}" class="form-horizontal form-label-left ajax-form">
        @csrf
        <input type="hidden" name="contract_id" value="{{ $data->contract->id }}">
        ...
        <div class="modal" id="editModal" tabindex="-1" role="dialog" aria-labelledby="editModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 style="font-weight:600;" class="modal-title" id="editModalLabel">Edit Physical Progress: {{ $data->name }}</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group row">
                            <label class="control-label col-md-4 col-sm-4">Activity Name</label>
                            <div class="col-md-8 col-sm-8">
                                <select name="activity" class="form-control" _e="1" disabled>
                                    <option value="">PLEASE CHOOSE</option>
                                    @foreach ($activities as $activity)
                                        <option value="{{ $activity->slug }}">{{ $activity->name }}</option>
                                    @endforeach
                        ...
                        <div class="form-group row">
                            <label class="control-label col-md-4 col-sm-4">Stage Name</label>
                            <div class="col-md-8 col-sm-8">
                                <select name="stage" class="form-control d-none" disabled></select>
                                <input class="form-control aph" disabled placeholder="Please Select an activity first">
                                {{-- <input type="text" class="form-control" name="stage_name" data-key="stage_name" placeholder="Stage Name.."> --}}
                                <p class="error" id="editerror-stage_name"></p>
                            </div>
                        ...
                        <div class="form-group row">
                            <label class="control-label col-md-4 col-sm-4">Add Images</label>
                            <div class="col-md-8 col-sm-8">
                                <input type="file" name="images[]" class="form-control" multiple>
                                <p class="error" id="editerror-images"></p>
                            </div>
